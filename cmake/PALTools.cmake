...
function(copy_asset_to_asset_pak_folder common_relative_path file_paths)
  foreach(file_path ${file_paths})
    set(asset_file_src ${CMAKE_CURRENT_SOURCE_DIR}/${common_relative_path}/${file_path})
    set(asset_file_dest ${LY_ASSET_OVERRIDE_PAK_FOLDER_ROOT})
    cmake_print_variables(asset_file_src)
    cmake_print_variables(asset_file_dest)
    file(COPY ${asset_file_src} DESTINATION ${asset_file_dest})
  endforeach()
endfunction()

function(copy_assets_to_raw_asset_folder common_relative_path file_paths target_platform_subfolder)
  foreach(file_path ${file_paths})
    set(asset_file_src ${CMAKE_CURRENT_SOURCE_DIR}/${common_relative_path}/${file_path})
    set(asset_file_dest ${CMAKE_HOME_DIRECTORY}/RawAssets/${target_platform_subfolder})
    cmake_print_variables(asset_file_src)
    cmake_print_variables(asset_file_dest)
    file(COPY ${asset_file_src} DESTINATION ${asset_file_dest})
  endforeach()
