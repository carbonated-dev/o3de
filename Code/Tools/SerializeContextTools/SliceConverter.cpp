    ...
    AZ_POP_DISABLE_WARNING
                            ...
                            if (linkRef.has_value())
                            {
                                         ...
                                         overrideType.has_value() && overrideType == AzToolsFramework::Prefab::OverrideType::AddEntity)
                                {
                                    ...
                                    rapidjson::Value path = rapidjson::Value(
                                        entityPathFromTopInstance.data(),
                                        aznumeric_caster(entityPathFromTopInstance.length()),
                                        overridePatches.GetAllocator());
                                ...
                                else
                                {
                                    AZ_Warning(
                                        "Prefab",
                                        false,
                                        "SliceConverter - Entity ('%s') is not present in template for override editing. Skipping it.",
                                ...
                                if (cachedOwningInstanceDom.has_value())
                                {
                                    ...
                                    {
                                        AzToolsFramework::Prefab::PrefabDomValue endStateCopy(
                                            entityDomAfterUpdate, cachedOwningInstanceDom->get().GetAllocator());
            ...
            AZ::SerializeContext* serializeContext, bool isDryRun)
        {
            ...
            for (auto& slice : sliceList)
            {
                ...
                AZStd::list<DataForSliceInstancesReordering> slicesForReorderingList;
                ...
                // In such case we will have correct parenting but we will lost data for "Child Entity Order" component. Less evil
                    ...
                    for (auto& item : patches)
                    {
                        ...
                        for (auto& addressTypeElement : address)
                        {
                            ...
                            if (element.contains("::Parent Entity"))
                            {
                                ...
                                data.parentEntityAlias = AZStd::string::format("Instance_%s", parentEntityId.ToString().c_str());
                                slicesForReorderingList.emplace_back(data);
                                added = true;
                                break;
                            }
                        }
                ...
                for (auto &item : slicesForReorderingList)
                {
                    AZStd::string instanceAlias = GetInstanceAlias(*(item.instance));
