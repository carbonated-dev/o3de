...
#include <AzCoreAddon/Driller/Stream.h>
...
namespace AZ
{
    namespace Debug
    {
        ...
        //////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////////////////////////////////
        void DrillerOutputStream::Write(u32 name, const AZ::VectorFloat& vf)
        {
            float data;
            vf.StoreToFloat(&data);
            Write(name, data);
        }
        ...
        {
            float data[15];
            unsigned int dataSize = 15 * sizeof(float);// position (vector3), axises (3xvector3) and half (3xfloat)
            obb.GetPosition().StoreToFloat4(data);
            obb.GetAxisX().StoreToFloat4(&data[3]);
            obb.GetAxisY().StoreToFloat4(&data[6]);
            obb.GetAxisZ().StoreToFloat4(&data[9]);
            data[12] = obb.GetHalfLengthX();
            data[13] = obb.GetHalfLengthY();
            data[14] = obb.GetHalfLengthZ();
            StreamEntry de;
            de.name = name;
            de.sizeAndFlags = dataSize;
            WriteBinary(de);
        ...
        void DrillerOutputStream::Write(u32 name, const AZ::Transform& /* tm */)
        {
            ...
            // Gruber patch begin // FIXME // AVK
            // No Transform::StoreToRowMajorFloat12
            AZ_Assert(false, "To be fixed");
            //tm.StoreToRowMajorFloat12(data);
...
#if defined(AZ_FILE_STREAM_COMPRESSION)
            // Gruber patch begin // FIXME // AVK
            // 'GetAllocator': is not a member of 'AZ::AllocatorInstance<AZ::OSAllocator>'
            AZ_Assert(false, "To be fixed");
        ...
        void DrillerSAXParser::Data::Read(AZ::VectorFloat& vf) const
        {
            float data;
            Read(data);
            vf = AZ::VectorFloat(data);
        }
        ...
        {
            AZ_Assert(m_dataSize == sizeof(float) * 15, "We are expecting 15 floats for Obb element 0x%08x with size %d bytes", m_name, m_dataSize);
            float* data = reinterpret_cast<float*>(m_data);
            if (m_isEndianSwap)
            {
                AZStd::endian_swap(data, data + 15);
                m_isEndianSwap = false;
            }

            // Gruber patch begin // FIXME // AVK
            // Vector3 position = Vector3::CreateFromFloat3(data);
            // Vector3 axisX = Vector3::CreateFromFloat3(&data[3]);
            // Vector3 axisY = Vector3::CreateFromFloat3(&data[6]);
            // Vector3 axisZ = Vector3::CreateFromFloat3(&data[9]);

            //float halfX = data[12];
            //float halfY = data[13];
            //float halfZ = data[14];
            // No Obb::CreateFromPositionAndAxes
            AZ_Assert(false, "To be fixed");
            //obb = Obb::CreateFromPositionAndAxes(position, axisX, halfX, axisY, halfY, axisZ, halfZ);
            // Gruber patch end // AVK
        }
        void DrillerSAXParser::Data::Read(AZ::Transform& /* tm */) const
        {
            ...
            // Gruber patch begin // FIXME // AVK
            // No Transform::CreateFromRowMajorFloat12
            AZ_Assert(false, "To be fixed");
            //tm = Transform::CreateFromRowMajorFloat12(data);
            // Gruber patch end // AVK
        }
        ...
        DrillerDOMParser::OnTag(AZ::u32 name, bool isOpen)
        {
            if (isOpen)
            {
                m_topNode->m_tags.emplace_back(); // was push_back();
