...
namespace GridMate
{
    namespace SocketOperations
    {
        ...
        Driver::ResultCode Send(SocketDriverCommon::SocketType sock, const char* buf, AZ::u32 bufLen, AZ::u32& bytesSent)
        {
            ...
            AZ::s32 result = (AZ::s32)send(sock, buf, static_cast<AZ::u32>(bufLen), AZ_TRAIT_GRIDMATE_MESSAGE_NO_SIGNAL);
            if (Platform::IsSocketError(result))
            {
                AZ::s32 err = Platform::GetSocketError();
                if (err != AZ_EWOULDBLOCK)
                {
                    AZ_Info("GridMate", "send() err:%d -> %s\n", err, AZ::AzSock::GetStringForError(err));
                    return Driver::EC_SEND;
                }
            }
            else
        ...
        Driver::ResultCode Receive(SocketDriverCommon::SocketType sock, char* buf, AZ::u32& inOutlen)
        {
            ...
            AZ::s32 result = (AZ::s32)recv(sock, buf, static_cast<AZ::u32>(inOutlen), 0);
            if (Platform::IsSocketError(result))
            {
                inOutlen = 0;
                AZ::s32 err = Platform::GetSocketError();
        ...
        GridMate::Driver::ResultCode Connect(SocketDriverCommon::SocketType sock, const sockaddr* socketAddress, size_t sockAddrSize, ConnectionResult& outConnectionResult)
        {
            ...
            AZ_Info("GridMate", "Connect() error:%d\n", Platform::GetSocketError());
        ...
        bool IsWritable(SocketDriverCommon::SocketType sock, AZStd::chrono::microseconds timeOut)
        {
            ...
            int result = ::poll(&fdPollData, 1, timeOut.count() / 1000);
#else
            fd_set fdwrite;
            FD_ZERO(&fdwrite);
            FD_SET(sock, &fdwrite);
    ...
    bool SocketAddressInfo::Resolve(char const* address, AZ::u16 port, Driver::BSDSocketFamilyType familyType, bool isDatagram, AdditionalOptionFlags flags)
    {
        ...
        if (error != 0)
        {
            SockerErrorBuffer buffer;
            AZ_UNUSED(buffer);
            AZ_Info("GridMate", "SocketDriver::Initialize - Platform::GetAddressInfo() failed with code %s at port %d\n", Platform::GetSocketErrorString(Platform::GetSocketError(), buffer), port);
            return false;
        }
        return true;
    }
        ...
        , m_isHighPerformance(isHighPerformance)
    {
        ...
        string ipv4AddressNonLocal = "192.168.0.1";
        string ipv6AddressNonLocal = "";
        IPv4ToIPv6(ipv4AddressNonLocal, ipv6AddressNonLocal);

        m_sock4AddressNonLocal = inet_addr(ipv4AddressNonLocal.c_str());
        ...
        char strPort[8];
        azsnprintf(strPort, AZ_ARRAY_SIZE(strPort), "%d", m_port); 

        int error = Platform::GetAddressInfo(ipv6AddressNonLocal.c_str(), strPort, &hints, &addrInfo);
        if (error != 0)
        {
            SockerErrorBuffer buffer;
            AZ_UNUSED(buffer);
            AZ_Assert(false, "SocketDriverCommon::SocketDriverCommon - Platform::GetAddressInfo() failed with %s!", Platform::GetSocketErrorString(Platform::GetSocketError(), buffer));
        }

        m_sock6AddressNonLocal = reinterpret_cast<const sockaddr_in6*>(addrInfo->ai_addr)->sin6_addr;
#endif
    }
    ...
    SocketDriverCommon::SetSocketOptions(bool isBroadcast, unsigned int receiveBufferSize, unsigned int sendBufferSize)
    {
        ...
        if (Platform::IsSocketError(ioctlsocket(m_socket, SO_NBIO, &sock_ctrl)))
        {
            ...
            (void)error;
            AZ_Info("GridMate", "SocketDriver::Initialize - ioctlsocket failed with code %d\n", error);
    ...
    SocketDriverCommon::Initialize(int ft, const char* address, unsigned int port, bool isBroadcast, unsigned int receiveBufferSize, unsigned int sendBufferSize)
    {
        ...
        if (Platform::IsValidSocket(m_socket))
        {
            ...
            if (Platform::IsSocketError(BindSocket(addrInfo->ai_addr, addrInfo->ai_addrlen)))
            {
                ...
                (void)socketErr;
                AZ_Info("GridMate", "SocketDriver::Initialize - bind failed with code %d at port %d\n", socketErr, port);
                Platform::FreeAddressInfo(addrInfo);
                closesocket(m_socket);
            ...
            if (m_port == 0)  // if we use implicit bind, retrieve the system assigned port
            {
                ...
                if (m_port == 0)
                {
                    int socketErr = Platform::GetSocketError();
                    (void)socketErr;
                    AZ_Info("GridMate", "SocketDriver::Initialize - getsockname failed with code %d at port %d\n", socketErr, port);
                    Platform::FreeAddressInfo(addrInfo);
                    closesocket(m_socket);
                    return EC_SOCKET_BIND;
                }
        ...
        else
        {
            ...
            int socketErr = Platform::GetSocketError();
            (void)socketErr;
            AZ_Info("GridMate", "SocketDriver::Initialize - socket failed with code %d at port %d\n", socketErr, port);
            Platform::FreeAddressInfo(addrInfo);
            return EC_SOCKET_CREATE;
        }
    ...
    SocketDriverCommon::Send(const AZStd::intrusive_ptr<DriverAddress>& to, const char* data, unsigned int dataSize)
    {
        ...
        if (m_canSend)
        {
            ...
            if (sockAddr == nullptr)
            {
                ...
                AZ_Info("GridMate", "SocketDriver::Send - address %s is not bound. This is not an error if you support unbound connectins, but data was NOT send!\n", to->ToString().c_str());
        ...
        {
            AZ_Info("GridMate", "SocketDriver::Send - Double Send for address %s\n", to->ToString().c_str());
    ...
    SocketDriverCommon::Receive(char* data, unsigned int maxDataSize, AZStd::intrusive_ptr<DriverAddress>& from, ResultCode* resultCode)
    {
        ...
        if (recvd)
        {
            ...
            if (!from)  // if we did not assign an address, ignore the data.
            {
                ...
                (void)port;
                AZ_Info("GridMate", "Data discarded from %s|%d\n", ip, port);
#endif // AZ_LOG_UNBOUND_SEND_RECEIVE
            }
    ...
    unsigned int SocketDriverCommon::PlatformSocketDriver::Receive(char* data, unsigned maxDataSize, sockaddr* sockAddr, socklen_t sockAddrLen, ResultCode* resultCode)
    {
        ...
        do
        {
            ...
            if (Platform::IsSocketError(recvd))
            {
                ...
                (void)error;
                AZ_Info("GridMate", "SocketDriver::Receive - recvfrom failed with code %d, dataSize=%d\n", error, maxDataSize);
                if (resultCode)
                {
...
#if defined(CARBONATED)
                sockAddr.sin6_addr = m_parent.m_sock6AddressNonLocal;
#else
...
#if defined(CARBONATED)
                //sockAddr.sin_addr.s_addr = (in_addr_t)m_parent.m_sock4AddressNonLocal;
                sockAddr.sin_addr.s_addr = decltype(sockAddr.sin_addr.s_addr)(m_parent.m_sock4AddressNonLocal);
#else
    ...
    bool SocketDriverCommon::RIOPlatformSocketDriver::isSupported()
    {
        ...
        if(!IsWindows8OrGreater())
        {
            ...
            AZ_Info("GridMate", "RIO not supported on this platform\n");
            return false;
        }
        return true;
    }
    ...
    Driver::ResultCode SocketDriverCommon::RIOPlatformSocketDriver::Initialize(unsigned int receiveBufferSize, unsigned int sendBufferSize)
    {
        ...
        AZ_Info("GridMate", "SocketDriver RIO (%p) starting up.\n", this);
        ...
        AZ_Info("GridMate", "SocketDriver RIO (%p) startup successful.\n", this);
    ...
    Driver::ResultCode SocketDriverCommon::RIOPlatformSocketDriver::Send(const sockaddr* sockAddr, unsigned int /*addressSize*/, const char* data, unsigned int dataSize)
    {
        ...
        {
            AZ_Info("GridMateSecure", "Buffer too large to send! Size=%u\n", dataSize);
