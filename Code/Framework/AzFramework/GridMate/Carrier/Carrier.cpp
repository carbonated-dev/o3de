    ...
    , m_datagramTempWriteBuffer(kCarrierEndian)
{
    ...
    m_lastMainThreadUpdate = AZStd::chrono::duration_cast<AZStd::chrono::milliseconds>(AZStd::chrono::system_clock::now().time_since_epoch()).count(); // Gruber patch // AVK
        ...
        if (!connection->m_isDisconnecting && connection->m_mainConnection->m_state != Carrier::CST_CONNECTING /* NAT punch allowance */)
        {
            ...
            if (m_enableDisconnectDetection && !connection->m_isBadConnection)
            {
                ...
                if (AZStd::chrono::milliseconds(m_currentTime - connection->m_lastReceivedDatagramTime).count() > connectionTimeout)
                {
                    ...
                    // Gruber patch // Replaced AZ_Info by AZ_Printf to make sure the messages go to the log file.
                    AZ_Printf("GridMate", "[%08x] We have NOT received packet from %s for %d ms. Connection is lost!\n", this, connection->m_target->ToString().c_str(), connectionTimeout);
                }
        ...
        else
        {
            ...
            else if (m_enableDisconnectDetection && !connection->m_isBadConnection)
            {
                ...
                if (timeNotReady.count() > m_handshakeTimeoutMS)
                {
                    ...
                    AZ_Printf("GridMate", "[%08x] Handshake to %s did not complete within %d ms!\n", this, connection->m_target->ToString().c_str(), m_handshakeTimeoutMS);
...
bool CarrierThread::SendDatagram(ThreadConnection* connection)
{
    ...
    {
        AZ_Printf("Carrier", "Send error: %d\n", (int)sendResult);
        ThreadMessage* mtm = aznew ThreadMessage(MTM_ON_ERROR);
        mtm->m_connection = connection->m_mainConnection;
...
CarrierThread::ThreadPump()
{
    while (m_quitThread == false)
    {
        ...
        AZ::u64 currentTimeStampMS = AZStd::chrono::duration_cast<AZStd::chrono::milliseconds>(m_currentTime.time_since_epoch()).count(); // Gruber patch // AVK
            ...
            while ((msg = PopCarrierThreadMessage()) != NULL)
            {
                ...
                case CTM_CONNECT:
                {
                    ...
                    if (!conn)
                    {
                        ...
                        {
                            AZ_Printf("GridMate", "Thread connection to %s already exists!\n", driverAddress->ToString().c_str());
        ...
        const auto currentTimeStampMS1 = AZStd::chrono::duration_cast<AZStd::chrono::milliseconds>(AZStd::chrono::system_clock::now().time_since_epoch()); // Gruber patch // AVK
        AZStd::chrono::milliseconds updateTime = currentTimeStampMS1 - AZStd::chrono::milliseconds(currentTimeStampMS);
        ...
        else
        {
            ...
            /*AZ_Info("GridMate", "Carrier thread %p updated for %u ms which is >= than thread update time %u ms! Yield will be called!\n",
...
CarrierThread::SendSystemMessage(SystemMessageId id, WriteBuffer& wb, ThreadConnection* target)
{
    ...
    {
        AZ_Printf("GridMate", "Discarding outbound system message 0x%x. Can't send any more system messages because the main connection has been torn down.\n", id);
        return;
    }
    ...
    // Process messages from the carrier thread
    {
        ...
        while ((msg = m_thread->PopMainThreadMessage()) != NULL)
        {
            ...
            case MTM_ON_ERROR:
            {
                if (msg->m_connection)
                {
                    AZ_Printf("GridMate", "Carrier::Connection %s had an error %d with error code %d\n",
                ...
                {
                    AZ_Printf("GridMate", "Carrier::Error %d with error code %d\n",
                        msg->m_errorCode,
                        (msg->m_errorCode == CarrierErrorCode::EC_DRIVER) ? static_cast<int>(msg->m_error.m_driverError.m_errorCode) : static_cast<int>(msg->m_error.m_securityError.m_errorCode));
                }
...
{
    m_currentTime = AZStd::chrono::system_clock::now();
    m_thread->m_lastMainThreadUpdate = AZStd::chrono::duration_cast<AZStd::chrono::milliseconds>(m_currentTime.time_since_epoch()).count(); // Gruber patch // AVK
...
CarrierImpl::DebugStatusReport(ConnectionID id, unsigned char channel)
{
    ...
    AZ_Printf("GridMate","=========================================================\n");
    AZ_Printf("GridMate","Connection %d status for channel %d\n",id,channel);
    AZ_Printf("GridMate","IsReady: %s isDisconn: %s\n",ToString(conn->isReady),ToString(conn->isDisconnecting));
    AZ_Printf("GridMate","Send: seqNum: %d reliableSeqNum: %d\n",conn->m_sendSeqNum[channel],conn->m_sendReliableSeqNum[channel]);
    AZ_Printf("GridMate","Is data to send %s (sorted on send priority):\n",ToString(HasDataToSend(conn)));
    ...
    {
    AZ_Printf("GridMate","  Dgram %d\n",*iter);
    }
    timeElapsed = now - conn->lastReceivedDatagramTime;
    AZ_Printf("GridMate","Time since last dgram %d ms\n",timeElapsed.count());
    AZ_Printf("GridMate","Time since connection start %d s\n",AZStd::chrono::seconds(now-conn->m_createTime).count());
...
CarrierImpl::DebugDeleteConnection(ConnectionID id)
{
    ...
    AZ_Printf("GridMate", "DebugDeleteConnection %s - SHOULD BE CALLED IN DEBUG TESTS ONLY!\n", conn->m_fullAddress.c_str());
    // Called to execute the necessary callbacks, otherwise the connection will be torn down instantly.
    DisconnectRequest(conn, reason);
