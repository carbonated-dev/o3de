...
namespace GridMate
{
    ...
    namespace ConnectionSecurity
    {
        ...
        AZ_INLINE static TOut UnpackByte(ReadBuffer& input)
        {
            ...
            AZ_Info("GridMateSecure", "Read buffer unpacked too many bytes.");
            return 0;
        }
        ...
        AZ_INLINE static void UnpackRange(ReadBuffer& input, AZ::u8 minBytes, AZ::u8 maxBytes, AZ::u8* output, AZ::u8& outputSize)
        {
            if (input.IsValid())
            {
                ...
                {
                    AZ_Info("GridMate", "Unpack out of range");
                }
                if (outputSize > 0)
                {
    ...
    SecureSocketDriver::Connection::~Connection()
    {
        ...
        {
            AZ_Info("GridMate", "DTOR Disconnected LOG: %s\n", m_dbgLog.c_str());
        }
#endif
        Shutdown();
    }
    ...
    bool SecureSocketDriver::Connection::OnStateEstablished(AZ::HSM& sm, const AZ::HSM::Event& event)
    {
        ...
        case AZ::HSM::EnterEventId:
        {
                ...
                ++count;
                AZ_Info("GridMateSecure", "OnStateEstablished %d\n", count);*/
...
#ifdef AZ_DebugUseSocketDebugLog
            AZ_Info("GridMate", "Disconnected LOG: %s\n", m_dbgLog.c_str());
#endif
            return true;
        }
        return false;
    ...
    Driver::ResultCode SecureSocketDriver::Initialize(AZ::s32 ft, const char* address, AZ::u32 port, bool isBroadcast, AZ::u32 receiveBufferSize, AZ::u32 sendBufferSize)
    {
        ...
        {
            AZ_Info("GridMateSecure", "If a private key is provided, so must a corresponding certificate.\n");
            return EC_SECURE_CONFIG;
        }

        if (m_desc.m_certificatePEM != nullptr && m_desc.m_privateKeyPEM == nullptr)
        {
            AZ_Info("GridMateSecure", "If a certificate is provided, so must a corresponding private key.\n");
            return EC_SECURE_CONFIG;
        }
        ...
        {
            AZ_Info("GridMateSecure", "No certificateAuthorityPEM set, using NULL verifier.\n");
        ...
        {
            AZ_Info("GridMateSecure", "Failed to set driver for ssl context\n");
            return EC_SECURE_CREATE;
        }
    ...
    int SecureSocketDriver::GenerateCookie(AddrPtr endpoint, unsigned char* cookie, unsigned int* cookieLen)
    {
        ...
        *cookieLen = 0;
        ...
        if (cookie == nullptr)
        {
            AZ_Info("GridMateSecure", "Cookie is nullptr, it needs to be allocated");
            return 0;
        }

        if (!RotateCookieSecret())
        {
            AZ_Info("GridMateSecure", "Failed to rotate secret\n");
            return 0;
        }
        ...
        {
            AZ_Info("GridMateSecure", "Insufficient cookie buffer: %u > %u\n", resultLen, cookieMaxLen);
            return 0;
        }
    ...
    int SecureSocketDriver::VerifyCookie(AddrPtr endpoint, unsigned char* cookie, unsigned int cookieLen)
    {
        if (cookie == nullptr)
        {
            AZ_Info("GridMateSecure", "Cookie is nullptr");
            return 0;
        }

        if (!m_cookieSecret.m_isCurrentSecretValid)
        {
            AZ_Info("GridMateSecure", "Secret not initialized, can't verify cookie\n");
            return 0;
        }
        ...
        AZ_Info("GridMate", "Failed to validate the cookie for %s\n", Internal::SafeGetAddress(addrStr) );
        return 0;
    }
