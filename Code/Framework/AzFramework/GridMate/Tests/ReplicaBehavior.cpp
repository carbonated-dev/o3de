    ...
    */
    ...
    public:
        ...
        void ResetCounts(bool trace = false)
        {
            ...
            {
                AZ_Info("GridMate", "Driller saw replicas with the following byte sizes:\n");
...
GM_TEST_SUITE(ReplicaBehaviorSuite)
GM_TEST(Integ_Replica_ComparePackingBoolsVsU8);
GM_TEST(Integ_ReplicaDefaultDataSetDriller);
GM_TEST(Integ_CheckDataSetStreamIsntWrittenMoreThanNecessary);
GM_TEST(Integ_CheckDataSetStreamIsntWrittenMoreThanNecessaryOnceDirty);
GM_TEST(Integ_Replica_DontSendDataSets_WithNoDiffFromCtorData);
GM_TEST(Integ_CheckReplicaIsntSentWithNoChanges);
GM_TEST(Integ_CheckEntityScriptReplicaIsntSentWithNoChanges);
GM_TEST_SUITE_END()
