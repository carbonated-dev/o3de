...
public:
    ...
    InterpolatorTest()
    {
        ...
        {
            m_zigVals[a] = sqrtf(static_cast< float >(a));
        }
    ...
    void run()
    {
        ...
        AZ_Info("GridMate", "this pointer: 0x%p\n", this);
        ...
        // testing linear interpolation and extrapolation
        {
            ...
            AZ_Info("GridMate", "this pointer: 0x%p", this);
        }
...
public:
    ...
    public:
        ...
        ReplicaChunkBase* CreateFromStream(UnmarshalContext& mc) override
        {
            ...
            {
                AZ_Info("GridMate", "Create with UserData:%p\n", mc.m_rm->GetUserContext(12345));
            }
    ...
    bool MyHandler123(const float& f, const RpcContext& rc)
    {
        (void)f;
        (void)rc;
        AZ_Info("GridMate", "Executed MyHandler123 requested at %u with %g on %s at %u.\n", rc.m_timestamp, f, GetReplica()->IsMaster() ? "Master" : "Proxy", rc.m_realTime);
        return true;
    }
...
protected:
    ...
    bool MyHandler2(const float& f, int p2, const RpcContext& rc)
    {
        ...
        (void)p2;
        (void)rc;
        AZ_Info("GridMate", "Executed MyHandler2 requested at %u with %g,%d on %s at %u.\n", rc.m_timestamp, f, p2, IsMaster() ? "Master" : "Proxy", rc.m_realTime);
        return true;
    }
    bool MyHandler3(const float& f, int p2, EBla p3, const RpcContext& rc)
    {
        ...
        (void)p3;
        (void)rc;
        AZ_Info("GridMate", "Executed MyHandler3 requested at %u with %g,%d,%d on %s at %u.\n", rc.m_timestamp, f, p2, p3, IsMaster() ? "Master" : "Proxy", rc.m_realTime);
        return true;
    }
...
public:
    ...
    {
        (void)rc;
        AZ_Info("GridMate", "NonMigratable replica 0x%x became %s on Peer %d\n", (int) GetReplicaId(), IsMaster() ? "master" : "proxy", (int) rc.m_rm->GetLocalPeerId());
    }
...
TEST_F(Integ_ReplicaGMTest, ReplicaTest)
    {
        ...
        while (keepRunning)
        {
            ...
            case 25:
                ...
                break;
            case 35:
                //AZ_Info("GridMate", "No more updates.\n");
                break;
            case 70:
                //AZ_Info("GridMate", "Restart updates.\n");
                break;
            case 90:
                keepRunning = false;
            }
    ...
    , public ::testing::Test
{
    void OnNewHost(bool isHost, ReplicaManager* pMgr) override
    {
        if (isHost)
        {
            AZ_Info("GridMate", "Peer %d has completed host migration and is now the host.\n", (int)pMgr->GetLocalPeerId());
            pMgr->SetSendTimeInterval(k_hostSendRateMs);
            m_newHostEventOnNewHostCount++;
        }
        else
        {
            AZ_Info("GridMate", "Peer %d has has received notification that host migration is complete.\n", (int)pMgr->GetLocalPeerId());
    ...
    {
        AZ_Info("GridMate", "Replica 0x%08x got new owner %d on frame %d.\n", repId, (int)repMgr->GetLocalPeerId(), m_frameCount);
...
TEST_F(Integ_ForcedReplicaMigrationTest, ForcedReplicaMigrationTest)
    {
        ...
        while (m_frameCount < k_numFramesToRun)
        {
            ...
            if (!allReady)
            {
                ...
                if (allReady)
                {
                    AZ_Info("GridMate", "All peers ready at frame %d\n", m_frameCount);
                }
            }
            ...
            if (allReady)
            {
                ...
                if (addReplicas)
                {
                    ...
                    addReplicas = false;
                    AZ_Info("GridMate", "Replicas added at frame %d\n", m_frameCount);
                }
                ...
                if (m_frameCount > 50 && dropP3)
                {
                    ...
                    dropP3 = false;
                    AZ_Info("GridMate", "Dropped P3 at frame %d\n", m_frameCount);
                }
                ...
                if (m_frameCount > 100 && dropP1)
                {
                    ...
                    dropP1 = false;
                    AZ_Info("GridMate", "Dropped P1 at frame %d\n", m_frameCount);
                }
                ...
                if (m_frameCount > 150 && promoteP2)
                {
                    ...
                    promoteP2 = false;
                    AZ_Info("GridMate", "Promoted P2 at frame %d\n", m_frameCount);
                }
            }
...
public:
    ...
    public:
        ...
        {
            AZ_Info("GridMate", "Node %d accepted transfer of AlwaysMigratable 0x%x to node %d.\n", rc.m_rm->GetLocalPeerId() - 1, GetReplicaId(), requestor - 1);
            m_requests++;
            m_accepted++;
        ...
        void OnReplicaChangeOwnership(const ReplicaContext& rc) override
        {
            if (IsMaster())
            {
                AZ_Info("GridMate", "OnChangeOwnership: 0x%04x Became master on node %d\n", GetReplicaId(), rc.m_rm->GetLocalPeerId() - 1);
                m_owner.Set(rc.m_rm->GetLocalPeerId() - 1);
            }
            else
            {
                AZ_Info("GridMate", "OnChangeOwnership: 0x%04x Became proxy on node %d\n", GetReplicaId(), rc.m_rm->GetLocalPeerId() - 1);
    ...
    public:
        ...
        virtual bool AcceptChangeOwnership(PeerId requestor, const ReplicaContext& rc) override
        {
            ...
            {
                AZ_Info("GridMate", "Node %d accepted transfer of SometimesMigratable 0x%x to node %d.\n", rc.m_rm->GetLocalPeerId() - 1, GetReplicaId(), requestor - 1);
                m_accepted++;
                return true;
            }
            return false;
        ...
        while (frameCount < framesToRun)
        {
            ...
            if (allReady)
            {
                ...
                if (frameCount > 50 && dropP2)
                {
                    ...
                    dropP2 = false;
                    AZ_Info("GridMate", "Dropped P2 at frame %d\n", frameCount);
                }
                ...
                if (frameCount > 100 && reconP2)
                {
                    ...
                    reconP2 = false;
                    AZ_Info("GridMate", "Reconnected P2 at frame %d\n", frameCount);
                }
                ...
                if (frameCount > 150 && redropP2)
                {
                    peers[p2].GetTransport()->Disconnect(AllConnections);
                    redropP2 = false;
                    AZ_Info("GridMate", "Re-Dropped P2 at frame %d\n", frameCount);
                }
            }
