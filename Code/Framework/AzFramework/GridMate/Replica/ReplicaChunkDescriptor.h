...
namespace GridMate
{
    ...
    */
    class AZF_API ReplicaChunkDescriptor
    {
    ...
    */
    class AZF_API ReplicaChunkDescriptorTable
    {
    public:
        ...
        bool RegisterChunkType()
        {
            // Gruber patch // Added logging.
            const AZStd::string chunkName(ReplicaChunkType::GetChunkName());
            ReplicaChunkClassId chunkTypeId(chunkName);
            ReplicaChunkDescriptor* descriptor = FindReplicaChunkDescriptor(chunkTypeId);

            AZ_Info("GridMate", "Registering Replica type %s(0x%x).\n", chunkName.c_str(), static_cast<unsigned int>(chunkTypeId));

            if (descriptor)
            {
                // TODO: verify descriptor compatibility
                AZ_Info("GridMate", "Replica type %s(0x%x) already registered. New registration ignored.\n", chunkName.c_str(), static_cast<unsigned int>(chunkTypeId));
            }
            else
            {
