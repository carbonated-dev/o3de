...
namespace GridMate
{
    ...
    AZ::u32 ReplicaChunkBase::CalculateDirtyDataSetMask(MarshalContext& mc)
    {
        ...
        else if ((mc.m_marshalFlags & ReplicaMarshalFlags::OmitUnmodified))
        {
            ...
            dataSetMask = (AZ::u32)m_nonDefaultValueBits.to_ulong();
        }
    ...
    void ReplicaChunkBase::MarshalDataSets(MarshalContext& mc, AZ::u32 chunkIndex)
    {
        ...
        mc.m_outBuffer->Write(static_cast<AZ::u32>(changebits.to_ulong()), VlqU32Marshaler());  // u32 to avoid type conversion error
#else        
        ...
        AZ_Printf("ReplicaChunkBase", "MarshalDataSets. Count=%llu", descriptor->GetDataSetCount());
#endif
        for (size_t i = 0; i < descriptor->GetDataSetCount(); ++i)
        {
            if (changebits[i])
    ...
    void ReplicaChunkBase::UnmarshalDataSets(UnmarshalContext& mc, AZ::u32 chunkIndex)
    {
        ...
        if (changebits.any())
        {
            ...
            {
                AZ_Info("GridMate", "Received dataset updates for replica id %08x(%s) from unexpected peer.", GetReplicaId(), IsActive() && IsMaster() ? "master" : "proxy");
                if (IsMaster())
                {
...
#if defined(CARBONATED_ENGINE_LOG)
        AZ_Printf("ReplicaChunkBase", "MarshalRpcs. m_rpcQueue.size()=%llu", m_rpcQueue.size());
#endif
            ...
            for (AZ::u32 rpcsRead = 0; rpcsRead < rpcCount; ++rpcsRead)
            {
                SafeGuardRead(mc.m_iBuf, [this, &mc, &chunkIndex]()
                {
                    ...
                    if (isRpcValid)
                    {
                        ...
                        if (m_rpcQueue.full())
                        {
                            // we must delete the request to be dropped to avoid memory leak
                            Internal::RpcRequest* requestToDelete = *m_rpcQueue.begin();
