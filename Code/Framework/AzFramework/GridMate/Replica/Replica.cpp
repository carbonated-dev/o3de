...
#include <GridMate/GridMateGruberPatches.h>
...
namespace GridMate
{
    ...
    void Replica::OnDeactivate(const ReplicaContext& rc)
    {
        ...
        AZ_Printf("Replica", "OnDeactivate. m_chunks.size()=%i", m_chunks.size());
#endif
        for (auto chunk : m_chunks)
        {
            if (chunk)
    ...
    bool Replica::MigrationSuspendUpstreamFn(PeerId ownerId, AZ::u32 requestTime, const RpcContext& rpcContext)
    {
        ...
        if (IsProxy())
        {
            //AZ_Info("GridMate", "Received upstream suspend ack requested at %u for 0x%x from 0x%x.\n", requestTime, GetRepId(), ownerId);
            m_manager->AckUpstreamSuspended(GetRepId(), ownerId, requestTime);
        }
        else
        {
            //AZ_Info("GridMate", "Sending upstream suspend ack requested at %u for 0x%x.\n", requestTime, GetRepId(), ownerId);
        }
        return true;
    }
    //-----------------------------------------------------------------------------
    bool Replica::MigrationRequestDownstreamAckFn(PeerId ownerId, AZ::u32 requestTime, const RpcContext& rpcContext)
    {
        ...
        if (IsProxy())
        {
            //AZ_Info("GridMate", "Received downstream ack requested at %u for 0x%x from 0x%x .\n", requestTime, GetRepId(), ownerId);
            m_manager->AckDownstream(GetRepId(), ownerId, requestTime);
        }
        else
        {
            m_flags |= Rep_SuspendDownstream;
            //AZ_Info("GridMate", "Sending downstream ack request at %u for 0x%x from 0x%x .\n", requestTime, GetRepId(), ownerId);
        }
        return true;
    }
    //-----------------------------------------------------------------------------
...
#if defined(CARBONATED_ENGINE_LOG)
        AZ_Printf("Replica", "  m_chunks.size=%u", m_chunks.size());
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("Replica", "  Chunk. replicaId=%u, Name=%s, Type=%i", chunk->GetReplicaId(), chunk->GetDescriptor()->GetChunkName(), chunk->GetDescriptor()->GetChunkTypeId());
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("Replica", "    chunkInfo.m_payload.GetExactSize()=%u", chunkInfo.m_payload.GetExactSize());
#endif
        }
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("Replica", "    chunkManifestBuffer.Size()=%u", chunkManifestBuffer.Size());
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
                AZ_Printf("Replica", "    chunkInfo.m_length.GetExactSize()=%u, chunkInfo.m_payload.GetExactSize()=%u", chunkInfo.m_length.GetExactSize(), chunkInfo.m_payload.GetExactSize());
#endif
            }
        }
    }
                ...
                if (chunkManifest.test(iChunk))
                {
...
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("Replica", "Unmarshal. iChunk=%llu, chunkSize=%llu", iChunk, chunkSize.GetSizeInBytesRoundUp());
#endif
                    ...
                    if (mc.m_hasCtorData)
                    {
                        ...
                        if (!chunk)
                        {
                            ...
                            AZ_Error("GridMate", chunk, "ERROR: Received unknown replica chunk type %u at index %d, discarding %d bytes and %d bits.",
                    ...
                    if (chunk)
                    {
                        chunkName = chunk->GetDescriptor()->GetChunkName();
                    }

                    AZ_Warning("GridMate", innerBuffer.IsEmpty() && !innerBuffer.IsOverrun(), "%s::Incorrect number of bytes read while unmarshaling chunk index %d, replica 0x%x. Data may be corrupted!", chunkName,  static_cast<int>(iChunk), GetRepId());
#else
    ...
    ReplicaChunkPtr Replica::CreateReplicaChunkFromStream(ReplicaChunkClassId classId, UnmarshalContext& mc)
    {
        ...
        {
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("Replica", "CreateReplicaChunkFromStream. Name=%s, Size=%u", pDesc->GetChunkName(), pDesc->GetChunkSize());
#endif
    ...
    {
#if defined(CARBONATED_ENGINE_LOG)
        AZ_Printf("CtorContextBase", "Marshal. m_members.size()=%llu", m_members.size());
#endif
