    ...
    namespace ReplicaInternal
    {
        ...
        bool MigrationSequence::OnStateMigrating(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_MIGRATING.\n", m_replica->GetRepId());
                m_pendingAcks.clear();
                return true;
        ...
        bool MigrationSequence::OnStateFlushUpstream(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_FLUSH_UPSTREAM.\n", m_replica->GetRepId());
        ...
        bool MigrationSequence::OnStateFlushDownstream(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_FLUSH_DOWNSTREAM.\n", m_replica->GetRepId());
                return true;
            case ME_UPDATE:
                for (auto& peerReplica : m_replicaMgr->m_peerReplicas)
                {
        ...
        bool MigrationSequence::OnStateChangeRouting(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_CHANGE_ROUTING_FOR_MIGRATION.\n", m_replica->GetRepId());
                return true;
            case ME_UPDATE:
            {
        ...
        bool MigrationSequence::OnStateHandoffReplica(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_HANDOFF_REPLICA.\n", m_replica->GetRepId());
                return true;
            case ME_PEER_REMOVED:
                if (*static_cast<PeerId*>(event.userData) == m_newOwnerId)
                {
        ...
        bool MigrationSequence::OnStateAbort(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_ABORT.\n", m_replica->GetRepId());
                return true;
            case ME_UPDATE:
        ...
        bool MigrationSequence::OnStateRollback(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_ROLLBACK.\n", m_replica->GetRepId());
                return true;
            case ME_UPDATE:
        ...
        bool MigrationSequence::OnStateChangeRoutingOnly(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_CHANGE_ROUTING_ONLY.\n", m_replica->GetRepId());
                return true;
            case ME_UPDATE:
            {
        ...
        bool MigrationSequence::DefaultHandler(AZ::HSM& sm, const AZ::HSM::Event& event)
        {
            ...
            {
            case AZ::HSM::EnterEventId:
                //AZ_Info("GridMate", "Replica 0x%x entering migration state MST_IDLE.\n", m_replica->GetRepId());
                return true;
            case ME_MODIFY_NEW_OWNER:
                m_newOwnerId = *static_cast<PeerId*>(event.userData);
        ...
        void MigrationSequence::OnReceivedAckUpstreamSuspended(PeerId from, AZ::u32 requestTime)
        {
            ...
            {
                //AZ_Info("GridMate", "Accepted upstream suspend ack response requested at %u for 0x%x from 0x%x.\n", requestTime, m_replica->GetRepId(), from);
                AZ::HSM::Event ev;
                ev.id = ME_PEER_ACK;
                ev.userData = &from;
                m_sm.Dispatch(ev);
        ...
        void MigrationSequence::OnReceivedAckDownstream(PeerId from, AZ::u32 requestTime)
        {
            if (requestTime == m_timestamp)
            {
                //AZ_Info("GridMate", "Accepted downstream ack response requested at %u for 0x%x from 0x%x.\n", requestTime, m_replica->GetRepId(), from);
                AZ::HSM::Event ev;
                ev.id = ME_PEER_ACK;
                ev.userData = &from;
                m_sm.Dispatch(ev);
