    ...
    */
    ...
    public:
        ...
        void Clear()
        {
            ...
            if (!m_tasks.empty())
            {
                AZ_Printf("ReplicaTaskManager", "Clear  m_tasks.size=%i", m_tasks.size());
            }
#endif
        ...
        void Run(ReplicaManager* replicaMgr)
        {
            ...
            {
                AZ_Printf("ReplicaTaskManager", "tasksToProcess.size=%u", tasksToProcess.size());
            }
#endif
            while (!tasksToProcess.empty())
            {
                ...
                {
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("ReplicaTaskManager", "  task->IsCancelled()");
#endif
                ...
                else
                {
                    ...
                    if (task->GetReplica())
                    {
                        AZ_Printf("ReplicaTaskManager", "  shouldProcess=%i, taskName=%s. ReplicaId=%u, (%s)", shouldProcess, task->GetTaskName().c_str(),
                            task->GetReplica().get()->GetRepId(), task->GetReplica().get()->GetDebugName());
                    }
                    else {
                        AZ_Printf("ReplicaTaskManager", "  shouldProcess=%i, taskName=%s. Replica is null", shouldProcess, task->GetTaskName().c_str());
                    }
#endif
                    ...
                    {
#if defined(CARBONATED_ENGINE_LOG)
                        AZ_Printf("ReplicaTaskManager", "SetAge=%u", age);
#endif
                        task->SetAge(age);
                        PriorityPolicy::UpdatePriority(*task);
                        Queue(task);
    ...
    private:
        ...
        void Queue(ReplicaTask* task)
        {
            ...
            if (task->GetReplica())
            {
                AZ_Printf("ReplicaTaskManager", "Queue. Replica=%i, (%s) m_tasks.size=%llu", task->GetReplica().get()->GetRepId(), task->GetReplica().get()->GetDebugName(), m_tasks.size());
            }
            else
            {
                AZ_Printf("ReplicaTaskManager", "Queue. Replica is null. m_tasks.size=%llu", m_tasks.size());
            }
#endif
        }
