...
namespace GridMate
{
    ...
    public:
        ...
        AZStd::string GetTaskName() override { return "ReplicaMarshalNewTask"; }

        //-----------------------------------------------------------------------------
        TaskStatus Run(const RunContext& context) override
        {
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("ReplicaMarshalNewTask", "Run. m_peer->IsOrphan()=%i", m_peer->IsOrphan());
#endif
            if (m_peer->IsOrphan())
            {
...
#if defined(CARBONATED_ENGINE_LOG)
            size_t bufferOffsetEnd0 = buffer.Size();
#endif
            AZ::u32 flags;
            switch (m_cmdId)
            {
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("ReplicaMarshalNewTask", "Run. from %i with length %i, totalCurrentSize=%i. NewReplicaSize=%i, m_cmdId=%i", bufferOffsetStart, (bufferOffsetEnd - bufferOffsetStart), bufferOffsetEnd, bufferOffsetEnd0, m_cmdId);
#endif
    ...
    public:
        ...
        AZStd::string GetTaskName() override { return "ReplicaMarshalUpstreamTask"; }
    ...
    public:
        ...
        AZStd::string GetTaskName() override { return "ReplicaMarshalZombieToPeerTask"; }
        ...
        TaskStatus Run(const RunContext& context) override
        {
            ...
            size_t bufferOffsetEnd = buffer.Size();
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("ReplicaMarshalZombieToPeerTask", "Run. from %i with length %i, totalCurrentSize=%i", bufferOffsetStart, (bufferOffsetEnd - bufferOffsetStart), bufferOffsetEnd);
#endif
            OnSendReplicaEnd(m_peer, buffer.Get() + bufferOffsetStart, bufferOffsetEnd - bufferOffsetStart);
    ...
    public:
        ...
        AZStd::string GetTaskName() override { return "ReplicaMarshalUpdateTask"; }
    ...
    ReplicaTask::TaskStatus ReplicaMarshalTask::Run(const RunContext& context)
    {
        ...
        for (auto& dst : m_replica->m_targets)
        {
            ...
            {
#if defined(CARBONATED_ENGINE_LOG)
                AZ_Printf("ReplicaMarshalTask", "IsNewOwner");
#endif
            ...
            {
#if defined(CARBONATED_ENGINE_LOG)
                AZ_Printf("ReplicaMarshalTask", "IsNew");
#endif
            ...
            {
#if defined(CARBONATED_ENGINE_LOG)
                AZ_Printf("ReplicaMarshalTask", "IsRemoved");
#endif
                WaitReplicaTask<ReplicaMarshalZombieToPeerTask>(context, m_replica, dst.GetPeer());
            }
            ...
            {
#if defined(CARBONATED_ENGINE_LOG)
                AZ_Printf("ReplicaMarshalTask", "isDownstreamDirty=%i, targetNeedsCallback=%i", isDownstreamDirty, targetNeedsCallback);
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("ReplicaMarshalTask", "CanUpstream()=%i, pdr.m_isUpstreamReliableDirty=%i, pdr.m_isUpstreamUnreliableDirty=%i", CanUpstream(), pdr.m_isUpstreamReliableDirty, pdr.m_isUpstreamUnreliableDirty);
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
        AZ_Printf("ReplicaMarshalTask", "Run. m_replica->m_targets=%i, result=%i, isDownstreamDirty=%i", count, result, isDownstreamDirty);
#endif
        return result;
    }
} // namespace GridMate
