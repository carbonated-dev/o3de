    ...
    namespace ReplicaInternal
    {
        ...
        */
        ...
        void SessionInfo::OnReplicaChangeOwnership(const ReplicaContext& rc)
        {
            ...
            m_pHostPeer = rc.m_peer;
            m_pHostPeer->MakeSyncHost(true);
            //AZ_Info("GridMate", "SystemReplica migrated from peerId=0x%x to peerId=0x%x.\n", m_formerHost, m_pHostPeer->GetId());
        }
        //-----------------------------------------------------------------------------
        ...
        {
            (void)rpcContext;
            //AZ_Info("GridMate", "Received upstream suspend ack response requested at %u for 0x%x from 0x%x.\n", requestTime, replicaId, peerId);
        ...
        {
            (void)rpcContext;
            //AZ_Info("GridMate", "Received downstream ack response requested at %u for 0x%x from 0x%x.\n", requestTime, replicaId, peerId);
