...
namespace GridMate
{
    ...
    // volume+edge lengths
    static AZ_FORCE_INLINE AZ::VectorFloat      size(const BvDynamicTree::VolumeType& a)
    {
    ...
    BvDynamicTree::split(const NodeArrayType& leaves,NodeArrayType& left,NodeArrayType& right,const AZ::Vector3& org,const AZ::Vector3& axis)
    {
        ...
        for(size_t i=0,ni=leaves.size();i<ni;++i)
        {
            ...
            //if(axis.Dot(leaves[i]->m_volume.GetCenter()-org).IsLessThan(AZ::VectorFloat::CreateZero()))
            if (axis.Dot(leaves[i]->m_volume.GetCenter() - org) < 0.0)
    ...
    BvDynamicTree::bottomup( NodeArrayType& leaves )
    {
        while(leaves.size()>1)
        {
            AZ::VectorFloat minsize = AZ::g_fltMax;
            int         minidx[2]={-1,-1};
            for(unsigned int i=0;i<leaves.size();++i)
            {
                for(unsigned int j=i+1;j<leaves.size();++j)
                {
                    const AZ::VectorFloat   sz=size(merge(leaves[i]->m_volume,leaves[j]->m_volume));
                    if(sz<minsize)
                    {
                        minsize     =   sz;
                        minidx[0]   =   i;
        ...
        if(leaves.size()>1)
        {
            if(leaves.size()>(unsigned int)bu_treshold)
            {
                ...
                for(unsigned int i=0;i<leaves.size();++i)
                {
                    ...
                    {
                        // Gruber patch begin // AVK
                        //++splitcount[j][x.Dot(axis[j]).IsGreaterThan(AZ::VectorFloat::CreateZero())?1:0];
                ...
                for(unsigned int i=0;i<3;++i)
                {
                    if((splitcount[i][0]>0)&&(splitcount[i][1]>0))
                    {
                        ...
                        const int   midp=(int)AZ::VectorFloat((float)(splitcount[i][0]-splitcount[i][1])).GetAbs();
                        if(midp<bestmidp)
                        {
                            bestaxis=i;
                            bestmidp=midp;
    ...
    //
    bool
    BvDynamicTree::Update(NodeType* leaf,VolumeType& volume,const AZ::Vector3& velocity,const AZ::VectorFloat& margin)
    {
    ...
    //
    bool
    BvDynamicTree::Update(NodeType* leaf,VolumeType& volume,const AZ::VectorFloat& margin)
    {
        if(leaf->m_volume.Contains(volume)) return(false);
        volume.Expand(AZ::Vector3(margin));
        Update(leaf,volume);
