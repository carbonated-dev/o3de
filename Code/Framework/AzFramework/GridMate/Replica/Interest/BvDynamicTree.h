...
#include <AzCoreAddon/Math/VectorFloat.h>
    ...
    namespace Internal
    {
        ...
        */
        ...
        public:
            ...
            AZ_FORCE_INLINE int                 Classify(const AZ::Vector3& n,const AZ::VectorFloat& o,int s) const
            {
                AZ::Vector3         pi,px;
                switch(s)
                {
                ...
                if( (n.Dot(px)+o).IsLessThan(zero) )            return -1;
                if( (n.Dot(pi)+o).IsGreaterEqualThan(zero) )    return  1;

                return 0;
            }
            AZ_FORCE_INLINE AZ::VectorFloat         ProjectMinimum(const AZ::Vector3& v,unsigned signs) const
            {
            ...
            AZ_FORCE_INLINE friend AZ::VectorFloat  Proximity(  const DynamicTreeAabb& a, const DynamicTreeAabb& b);
        ...
        private:
            AZ_FORCE_INLINE void                AddSpan(const AZ::Vector3& d,AZ::VectorFloat& smi,AZ::VectorFloat& smx) const
            {
        ...
        AZ_FORCE_INLINE bool        IntersectAabbPlane(const DynamicTreeAabb& a, const AZ::Plane& b)
        {
            ...
            // Gruber patch begin // AVK
            //return b.GetPointDist(testPoint).IsLessEqualThan(AZ::VectorFloat::CreateZero());
            return b.GetPointDist(testPoint) <= 0.0;
            // Gruber patch end // AVK
        }

        //
        AZ_FORCE_INLINE AZ::VectorFloat Proximity(  const DynamicTreeAabb& a, const DynamicTreeAabb& b)
        {
        ...
        {
            return Proximity(o,a).IsLessThan(Proximity(o,b));
        }
    ...
    */
    ...
    private:
        ...
        struct  sStkNPS
        {
            const NodeType* node;
            int         mask;
            AZ::VectorFloat value;
            sStkNPS() {}
            sStkNPS(const NodeType* n,unsigned m,const AZ::VectorFloat& v) : node(n),mask(m),value(v) {}
        };
        struct  sStkCLN
        {
            const NodeType* node;
    ...
    public:
        ...
        struct  ICollideCollector
        {
            ...
            void    Process(const NodeType* n,const AZ::VectorFloat&)   { Process(n); }
        ...
        bool            Update(NodeType* leaf,VolumeType& volume,const AZ::Vector3& velocity,const AZ::VectorFloat& margin);
        bool            Update(NodeType* leaf,VolumeType& volume,const AZ::Vector3& velocity);
        bool            Update(NodeType* leaf,VolumeType& volume,const AZ::VectorFloat& margin);
...
//              btAlignedObjectArray<const NodeType*>   stack;
        ...
        static void rayTest( const NodeType* root, const AZ::Vector3& rayFrom, const AZ::Vector3& rayTo, Collector& collector)
        {
            if(root)
            {
                ...
                // gruber patch beg
                // unsigned int signs[3] = { rayDirectionInverse.GetX() < 0.0f, rayDirectionInverse.GetY() < 0.0f, rayDirectionInverse.GetZ() < 0.0f };
                //signs[0] = rayDirectionInverse.GetX().IsLessThan(zero);
                //signs[1] = rayDirectionInverse.GetY().IsLessThan(zero);
                //signs[2] = rayDirectionInverse.GetZ().IsLessThan(zero);
                // gruber patch end

                //AZ::VectorFloat lambda_max = rayDir.Dot(ray);
                ...
                do  {
                    ...
                    //AZ::VectorFloat tmin = AZ::VectorFloat::CreateOne();
                    //AZ::VectorFloat lambda_min = zero;
                    // todo..
                    unsigned int result1 = /*btRayAabb2(rayFrom,rayDirectionInverse,signs,bounds,tmin,lambda_min,lambda_max)*/0;
#ifdef COMPARE_BTRAY_AABB2
                    AZ::VectorFloat param=AZ::VectorFloat::CreateOne();
        ...
        template<class Collector>
        void        rayTestInternal(const NodeType* root, const AZ::Vector3& rayFrom, const AZ::Vector3& rayTo, const AZ::Vector3& rayDirectionInverse, unsigned int signs[3],const AZ::VectorFloat& lambda_max, const AZ::Vector3& aabbMin, const AZ::Vector3& aabbMax, Collector& collector) const
        {
            ...
            if(root)
            {
                ...
                do
                {
                    ...
                    //AZ::VectorFloat tmin = AZ::VectorFloat::CreateOne();
                    //AZ::VectorFloat lambda_min = AZ::VectorFloat::CreateZero();
        ...
        template<class Collector>
        static void collideKDOP(const NodeType* root,   const AZ::Vector3* normals, const AZ::VectorFloat* offsets, int count, Collector& collector)
        {
...
/*          if(root)
        ...
        template<class Collector>
        static void collideOCL( const NodeType* root,   const AZ::Vector3* normals, const AZ::VectorFloat* offsets, const AZ::Vector3& sortaxis, int count, Collector& collector, bool fullsort=true)
        {
...
/*          if(root)
    ...
    private:
        ...
        // Helpers
        //static AZ_FORCE_INLINE int    nearest(const int* i,const BvDynamicTree::sStkNPS* a,const AZ::VectorFloat& v,int l,int h)
        //{
        //  int m=0;
        //  while(l<h)
        //  {
