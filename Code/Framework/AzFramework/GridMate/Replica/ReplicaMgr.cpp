...
namespace GridMate
{
    ...
    void ReplicaPeer::SendBuffer(Carrier* carrier, unsigned char commChannel, const AZ::u32 replicaManagerTimer)
    {
        ...
        if (m_reliableOutBuffer.Size() != 4 || m_unreliableOutBuffer.Size() != 4)
        {
            AZ_Printf("GridMate", "SendBuffer [0x%p]: ReliableSize=%llu, UnreliableSize=%llu", GetConnectionId(), m_reliableOutBuffer.Size(), m_unreliableOutBuffer.Size());
        }
#endif
        ...
        if (hasUnreliableData)
        {
            ...
            {
                AZ_Info("GridMate", "SendBuffer [0x%p]: Unreliable replica update exceeds MTU (size=%u, MTU=%u), forcing reliable for this send.\n", GetConnectionId(), static_cast<unsigned int>(m_unreliableOutBuffer.Size()), carrier->GetMessageMTU());
            }
    ...
    void ReplicaManager::Init(const ReplicaMgrDesc& desc)
    {
        ...
        {
            //AZ_Info("GridMate", "targetSendTimeMS is too high for underlying carrier!\n");
        ...
        {
// Gruber patch begin // FIXME // VMED -- missed type Descriptor in  AZ::PoolAllocator
#if 0
...
#endif
            AZ_Error("ReplicaManager", false, "AZ::PoolAllocator is not used in Gruber. It might be fixed");
// Gruber patch end // FIXME // VMED 
        }
    ...
    void ReplicaManager::Shutdown()
    {
        ...
        {
            //AZ_Info("GridMate", "ReplicaManager is already shutting down!\n");
            return;
        }
...
// Gruber patch begin // FIXME // VMED -- Destroy() is inaccessible
...
// Gruber patch end // FIXME // VMED
    ...
    void ReplicaManager::Promote()
    {
        ...
        //AZ_Info("GridMate", "Local peer id=0x%x promoted to sync host.\n", m_self.GetId());
        ...
        
        {
            ...
            for (ReplicaPeerList::iterator iPeer = m_remotePeers.begin(); iPeer != m_remotePeers.end(); ++iPeer)
            {
                ...
                if (peer->GetId() == orphanId)
                {
                    ...
                    {
#if defined(CARBONATED_ENGINE_LOG)
                        AZ_Printf("ReplicaManager", "RemoveReplicaFromDownstream from DiscardOrphans");
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
                        AZ_Printf("ReplicaManager", "RemoveReplicaFromDownstream from RemovePeer");
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
        AZ_Printf("ReplicaManager", "RemoveReplicaFromDownstream. Creating ReplicaMarshalZombieTask");
#endif
        ...
        while (!rb.IsEmptyIgnoreTrailingBits())
        {
            ...
            case Cmd_NewProxy:
            {
                ...
                if (pReplica)
                {
                    ...
                    {
                        //AZ_Info("Gridmate", "Received replica, id 0x%x, with old ownerSeq. Discarding %d bytes\n", (int)repId, (int)chunkSize);
                        break;
                    }
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("Gridmate", "Received replica, id %u. Size is %llu bytes", repId, chunkSize.GetSizeInBytesRoundUp());
#endif
                ...
                {
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("Gridmate", "Received and created replica, id %u. Size is %llu bytes", repId, chunkSize.GetSizeInBytesRoundUp());
#endif
            ...
            default:
            {
                ...
                if (!pObj)
                {
                    ...
                    {
                        AZ_Info("GridMate", "Received updates for tombstoned replica(id=0x%x).\n", repId);
                    }
                    else
                    {
                        AZ_Info("GridMate", "Received updates for unknown replica(id=0x%x). Maybe it has already been deleted?\n", repId);
                    }
                ...
                {
                    //AZ_Info("GridMate", "Skipping unmarshal for 0x%x because we can't identify the correct peer.\n", pObj->GetRepId());      
                }
...
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("GridMate", "ReceiveBuffer [Peer: %i]: Bytes=%u, roundUpBytes=%llu", peer->GetId(), result.m_numBytes, rb.Size().GetSizeInBytesRoundUp());
#endif
...
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("GridMate", "ReceiveBuffer [Peer: %i]: Bytes=%u, roundUpBytes=%llu. FINISHED PROCESSING", peer->GetId(), result.m_numBytes, rb.Size().GetSizeInBytesRoundUp());
#endif
                }
    ...
    ReplicaId ReplicaManager::AddMaster(const ReplicaPtr& pMaster)
    {
        ...
        //AZ_Info("GridMate", "Peer 0x%x: Added replica master 0x%x.\n",
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("ReplicaManager", "RemoveReplicaFromDownstream from Destroy. GetRepId=%i", requestor->GetRepId());
#endif
    ...
    void ReplicaManager::OnDestroyProxy(ReplicaId repId)
    {
        ...
        if (pObj)
        {
            ...
            {
                AZ_Info("GridMate", "Received non-authoritative request to destroy replica!");
                return;
            }
...
// Gruber patch begin // FIXME // VMED -- get_container() is not supported for AZStd::prioprity_queue
#if 0
...
// Gruber patch end // FIXME // VMED -- get_container() is not supported for AZStd::prioprity_queue

        if (updateRate)
        {
            //AZ_DbgIf(minRateBytesPerSecond < old)
            //{
            //    AZ_Info("GridMate", "%p updateConnection %p %u -> %u \n",
