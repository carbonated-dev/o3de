...
// Gruber patch begin // VMED -- missed args for push_back()
            m_vdt.push_back(ReplicaChunkMemberDescriptor());
...
// Gruber patch begin // VMED -- missed args for push_back()
            m_vrt.push_back(ReplicaChunkMemberDescriptor());
// Gruber patch end // VMED
...
#   define GRIDMATE_DESCRIPTOR_TABLE_VARIABLE_NAME AZ_CRC("GridMateReplicaChunkDescriptorTableForTools", 0x21eb3150)
#   define GRIDMATE_CHUNK_INIT_CONTEXT_STACK_VARIABLE_NAME AZ_CRC("GridMateReplicaChunkInitContextStackForTools", 0x8b16794b)
#else
...
// Gruber patch begin // FIXME // VMED -- missed IsReady()
        //if (AZ::AllocatorInstance<AZ::OSAllocator>::IsReady())
// Gruber patch end // FIXME // VMED
        {
            // Gruber patch begin // AVK // Deallocation of static objects allocated by AZ::OSAllocator is unsafe due to the order of static objects deletion.
            //TODO MADPORT-492
#if 0
    ...
    bool ReplicaChunkDescriptorTable::UnregisterReplicaChunkDescriptor(ReplicaChunkClassId chunkTypeId)
    {
        for (DescriptorContainerType::iterator itModuleTable = m_moduleDescriptorTable.begin(); itModuleTable != m_moduleDescriptorTable.end(); ++itModuleTable)
        {
            ...
            if (chunkTypeId == descInfo->m_chunkTypeId)
            {
                ...
                AZ_Info("GridMate", "Failed to find replica chunk descriptor in global table! Removing from local table.");
                return false;
            }
        }
        AZ_Info("GridMate", "Failed to find replica chunk descriptor in local table! Descriptor cannot be unregistered from this module!");
        return false;
    }
    //-----------------------------------------------------------------------------
    void ReplicaChunkDescriptorTable::AddReplicaChunkDescriptor(ReplicaChunkClassId chunkTypeId, ReplicaChunkDescriptor* descriptor)
    {
        ...
        AZ_Printf("ReplicaChunkDescriptorTable", "AddReplicaChunkDescriptor. chunkTypeId=%u, ChankName=%s", chunkTypeId, descriptor->GetChunkName());
#endif
    }
    //-----------------------------------------------------------------------------
    bool ReplicaChunkDescriptorTable::UnregisterReplicaChunkDescriptorFromGlobalTable(ReplicaChunkClassId chunkTypeId)
    {
        // Gruber patch begin // AVK // make sure m_globalDescriptorTable is constructed
        if (!m_globalDescriptorTable.IsConstructed())
        {
            return false;
        }
        // Gruber patch end
    ...
    {
// Gruber patch begin // VMED -- missed args for push_back()
        m_globalChunkInitContextStack->push_back(ReplicaChunkInitContext());
// Gruber patch end // VMED
