        ...
        PrepareDataResult PrepareData(EndianType endianType, AZ::u32 marshalFlags) override
        {
            ...
            if (ReplicaTarget::IsAckEnabled())
            {
                if (!IsWithinToleranceThreshold())
                {
                    ...
                    AZ_Printf("DataSet", "IsWithinToleranceThreshold. m_streamCache.size=%u", m_streamCache.Size());
#endif
                    ...
                    )
                {
                    ...
                    m_streamCache.Write(m_value, m_marshaler);
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("DataSet", "ForceDirty. m_streamCache.size=%u", m_streamCache.Size());
#endif
                }
            }
            else
            {
                ...
                if (isDirty)
                {
                    ...
                    m_streamCache.Write(m_value, m_marshaler);
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("DataSet", "isDirty. m_streamCache.size=%u", m_streamCache.Size());
#endif
                ...
                else if ((marshalFlags & ReplicaMarshalFlags::ForceDirty) || (marshalFlags & ReplicaMarshalFlags::OmitUnmodified))
                {
                    ...
                    */
                    ...
                    m_streamCache.Write(m_value, m_marshaler);
#if defined(CARBONATED_ENGINE_LOG)
                    AZ_Printf("DataSet", "last condition. m_streamCache.size=%u", m_streamCache.Size());
#endif
                }
            }
