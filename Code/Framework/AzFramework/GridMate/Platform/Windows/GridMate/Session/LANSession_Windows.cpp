...
namespace GridMate
{
    namespace Platform
    {
        void AssignExtendedName(GridMate::string& extendedName)
        {
            ...
            char procPath[256];
            char procName[256];
            // Gruber patch begin astarykh : explicitly call method using CHAR* buffer instead of a generic method, which needs WCHAR* buffer when UNICODE is defined // STA
            DWORD ret = GetModuleFileNameA(NULL, procPath, 256);
            // Gruber patch end astarykh // STA
            if (ret > 0)
            {
