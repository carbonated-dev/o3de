...
namespace GridMate
{
    ...
    struct AZF_API GridSessionParam
    {
        enum ValueType
        {
            VT_INT32,
    ...
    protected:
        ...
        static void* UserDataCopier(const void* sourceData, unsigned int sourceDataSize)
        {
            ...
            AZ_Assert(sizeof(T) == sourceDataSize, "Data size %d doesn't match the type size %d", sourceDataSize, sizeof(T))
                return azcreate(T, (*static_cast<const T*>(sourceData)), GridMateAllocatorMP);// , "UserDataCopier"); // Gruber patch // VMED -- Param #4 is redundant
        }
        // gruber patch beg
        template<class TT>
        ...
        {
            (void)dataSize;
            AZ_Assert(sizeof(TT) == dataSize, "Data size %d doesn't match the type size %d", dataSize, sizeof(TT));
            TT* data = static_cast<TT*>(pointer);
            azdestroy(data, GridMateAllocatorMP, TT);
        }
        // gruber patch end
