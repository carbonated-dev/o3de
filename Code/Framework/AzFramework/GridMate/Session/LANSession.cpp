...
bool LANSession::Initialize(const LANSessionParams& params, const CarrierDesc& carrierDesc)
{
    ...
    if (params.m_port != 0)
    {
        ...
        AZ_Printf("LANSession", "*** Server=%s, port=%i", params.m_address.c_str(), params.m_port);
#endif
...
LANSession::Update()
{
    ...
    if (m_driver)
    {
        ...
        while (true)
        {
            ...
            {
                AZ_Info("GridMate", "LANSession::Receive - recvfrom failed with code 0x%08x\n", result);
                break;
            }
            if (recvd == 0)
            {
            ...
            if (msg.m_message == LANSessionMsg::MT_SEARCH)
            {
                ...
                if (result != Driver::EC_OK)
                {
                    AZ_Info("GridMate", "LANSession::Send - sendto failed with code 0x%08x at %s", result, from->ToString().c_str());
                }
            }
            else
            {
...
// Gruber patch begin // FIXME // VMED -- uuid data is private
    //was: MemberIDCompact id = AZ::Crc32(&uuid.data, sizeof(uuid.data));
    MemberIDCompact id = AZ::Crc32(uuid.ToFixedString().c_str(), uuid.ToFixedString().length());
// Gruber patch begin // FIXME // VMED
    /////////////////////////////////////////////////////////////////////////////
    ...
    , m_searchParams(searchParams)
{
    ...
    m_driver = aznew SocketDriver(false, true);
    auto res = m_driver->Initialize(searchParams.m_familyType, searchParams.m_listenAddress.c_str(), searchParams.m_listenPort, true);
    if (res != Driver::EC_OK)
    {
#if defined(CARBONATED_ENGINE_LOG)
        AZ_Printf("LANSearch", "*** Result is %i. Not OK", res);
#endif
        SearchDone();
    }
...
LANSearch::Update()
{
    ...
    if (timeElapsed.count() >= m_broadcastInterval)
    {
        ...
        if (result != Driver::EC_OK)
        {
            AZ_Info("GridMate", "LANSearch::Send - to %s port %d failed with code 0x%08x\n", serverAddress.c_str(), m_searchParams.m_serverPort, result);
        }
    }
    ...
    while (true)
    {
        ...
        if (result != Driver::EC_OK)
        {
            AZ_Info("GridMate", "LANSearch::Receive - recvfrom failed with code 0x%08x\n", result);
            break;
        }
        if (recvd == 0)
        {
    ...
    : SessionService(desc)
{
    ...
    if (err != 0)
    {
        AZ_Info("GridMate", "Failed on WSAStartup with code %d\n", err);
    }
#endif
}
