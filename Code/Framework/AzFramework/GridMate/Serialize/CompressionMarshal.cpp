...
{
    AZ_Assert(AZ::IsClose(norQuat.GetLengthSq(),AZ::VectorFloat::CreateOne()), "Input quaternion is not normalized!");
...
QuatCompNormMarshaler::Unmarshal(AZ::Quaternion& quat, ReadBuffer& rb) const
{
    ...
    w = sqrtf(AZ::GetMax(0.f, 1.0f - x * x - y * y - z * z));
    if (flags & W_NEG)
    {
        w = -w;
    }
...
QuatCompNormQuantizedMarshaler::Unmarshal(AZ::Quaternion& quat, ReadBuffer& rb) const
{
    ...
    else
    {
        ...
        {
            AZ_Info("GridMate", "Error unmarshaling X angle for QuatCompNormQuantizedMarshaler! Aborting Unmarshal!\n");
            return;
        }
    ...
    else
    {
        if (!rb.Read(quantizedAngleFromNetwork))
        {
            AZ_Info("GridMate", "Error unmarshaling Y angle for QuatCompNormQuantizedMarshaler! Aborting Unmarshal!\n");
            return;
        }
        quantizedAngle = quantizedAngleFromNetwork;
    }
    ...
    else
    {
        if (!rb.Read(quantizedAngleFromNetwork))
        {
            AZ_Info("GridMate", "Error unmarshaling Z angle for QuatCompNormQuantizedMarshaler! Aborting Unmarshal!\n");
            return;
        }
        quantizedAngle = quantizedAngleFromNetwork;
    }
...
// Gruber patch begin // VMED
    if (!AZ::IsClose(scale.GetX(), AZ::VectorFloat::CreateOne()) || !AZ::IsClose(scale.GetY(), AZ::VectorFloat::CreateOne()) || !AZ::IsClose(scale.GetZ(),AZ::VectorFloat::CreateOne()))
...
// Gruber patch begin // FIXME // VMED -- missed SetRotationPartFromQuaternion
#if 0
        xform.SetRotationPartFromQuaternion(rot);
#endif
...
// Gruber patch begin // FIXME // VMED -- missed MultiplyByScale
#if 0
...
{
    AZ_Assert(value >= (m_min - AZ_FLT_EPSILON) && value <= (m_min + m_range + AZ_FLT_EPSILON), "Data is outside the range!");
