...
#include <AzFrameworkAddon/Network/NetSystemBus.h>
...
namespace AzFramework
{
    ...
    class AZF_API NetworkContextChunkDescriptor
    ...
    void NetworkContext::InitReflectedChunkBinding(ClassDescPtr binding)
    {
        if (!binding->RegisterChunkType)
        {
            ...
            binding->CreateReplicaChunk = [netContext, chunkClassId]()
                {
                    // gruber patch beg
                    ReflectedReplicaChunkBase* chunk = new(azmalloc(ReflectedReplicaChunk<ClassType>::GetChunkSize(), AZStd::alignment_of<ReflectedReplicaChunk<ClassType> >::value, AZ::SystemAllocator))ReflectedReplicaChunk<ClassType>();
                    // gruber patch end
