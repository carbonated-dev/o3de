...
namespace AzFramework
{
    void NetBindingComponent::Reflect(AZ::ReflectContext* reflection)
    {
        ...
        if (behaviorContext)
        {
            behaviorContext->EBus<NetBindingHandlerBus>("NetBindingHandlerBus")
                ->Attribute(AZ::Script::Attributes::ExcludeFrom, AZ::Script::Attributes::Unused) // Gruber patch begin // AVK // Original code was AZ::Script::Attributes::Preview
    ...
    void NetBindingComponent::Activate()
    {
        ...
        if (!IsEntityBoundToNetwork())
        {
            ...
            AZ_Printf("NetBindingComponent", "Activate. GetEntityId()=%s, '%s', shouldBind=%i, m_chunk=%0x", GetEntityId().ToString().c_str(), GetEntity()->GetName().c_str(), shouldBind, m_chunk.get());
#endif
            if (shouldBind)
            {
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("NetBindingComponent", "Deactivate. %s, %s, IsMaster=%i", GetEntityId().ToString().c_str(), GetEntity()->GetName().c_str(), m_chunk->IsMaster());
#endif
            if (m_chunk->IsMaster())
            {
...
#if defined(CARBONATED_ENGINE_LOG)
            AZ_Printf("NetBindingComponent", "Bound entity to the network. %s, '%s' ", this->GetEntityId().ToString().c_str(), this->GetEntity()->GetName().c_str());
#endif
    ...
    void NetBindingComponent::UnbindFromNetwork()
    {
        if (m_chunk)
        {
            ...
            // Gruber patch begin
            // It appears m_chunk can be nullified by the code above.
            if (!m_chunk)
            {
                AZ_Warning("NetBindingComponent", false, "UnbindFromNetwork(): m_chunk has been nullified.");
                return;
            }
            // Gruber patch end
            ...
            if (chunk->IsProxy())
            {
                ...
                {
                    NetBindingSystemBus::Broadcast( &NetBindingSystemBus::Events::UnbindGameEntity, GetEntityId(), m_spawnableInstanceId);  // Gruber patch. // LVB. // Was m_sliceInstanceId
                }
            }
        }
    }
    ...
    //}
    void NetBindingComponent::SetSpawnableInstanceId(const SpawnableInstanceId& spawnableInstanceId)
    {
