...
#include <AzFrameworkAddon/TargetManagement/TargetManagementComponent.h>
#include <AzFrameworkAddon/TargetManagement/NeighborhoodAPI.h>
...
namespace AzFramework
{
    ...
    public:
...
#endif
            m_gridMate = GridMate::GridMateCreate(desc);
        ...
        void OnMemberLeaving(GridMate::GridSession* session, GridMate::GridMember* member) override
        {
            ...
            if (session == m_session)
            {
                AZ_Info("TargetManager", "Member %p(%s) leaving TM session %p.\n", member, member->IsHost() ? "Host" : "Peer", session);
            }
        }
        //////////////////////////////////////////////////////////////////////////
        ...
        void OnNodeJoined(const Neighborhood::NeighborReplica& node) override
        {
            ...
            AZ_Info("Neighborhood", "Discovered node with 0x%x from member %d.\n", capabilities, static_cast<int>(memberId));
        ...
        {
            AZ_Info("Neighborhood", "Lost contact with node from member %d.\n", static_cast<int>(node.GetTargetMemberId()));
...
#ifndef GRIDMATE_FOR_TOOLS
        if (!AZ::AllocatorInstance<GridMate::GridMateAllocator>::IsReady())
        {
            AZ::AllocatorInstance<GridMate::GridMateAllocator>::Create();
        }

        if (!AZ::AllocatorInstance<GridMate::GridMateAllocatorMP>::IsReady())
        {
    ...
    {
        AZ_Info("TargetManagementComponent", "Set Target - %u", desiredTargetID);
    ...
    void TargetManagementComponent::SendTmMessage(const TargetInfo& target, const TmMsg& msg)
    {
        ...
        m_outboxMutex.lock();
        m_outbox.emplace_back(); // was push_back();
                            ...
                            while (result.m_state != result.NO_MESSAGE_TO_RECEIVE)
                            {
                                ...
                                if (target)
                                {
                                    ...
                                    if (msg)
                                    {
                                        if (msg->GetCustomBlobSize() > 0)
                                        {
                                            void* blob = azmalloc(msg->GetCustomBlobSize(), 1, AZ::OSAllocator); // was , "TmMsgBlob"
