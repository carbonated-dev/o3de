    ...
    void InitializeFromLuaStackFunctions(AZ::BehaviorContext* behaviorContext, AZStd::vector<DebugDataSource>& symbols)
    {
        for (auto& debugDataSource : symbols)
        {
                ...
                || debugDataSource.m_slotDatumType.IsValid())
            {
                ...
                // param.m_typeId.ToString<AZStd::string>().data())
                if (debugDataSource.m_fromStack == nullptr)
                {
                    AZ_Warning("ExecutionStateInterpretedUtilityCpp", false,
                               "LuaLoadFromStack function not found for type %s", param.m_typeId.ToString<AZStd::string>().data());
                }
                // Gruber patch end.
#else
